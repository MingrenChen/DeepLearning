...
class Model(object):
    ...
    def back_propagate(self, input_batch, activations, loss_derivative):
        ...
        ###########################   YOUR CODE HERE  ##############################
        # print("lost d", loss_derivative.shape)
        # print("embed_to_hid_weights",self.params.embed_to_hid_weights.shape)
        # print("hid_to_output_weights", self.params.hid_to_output_weights.shape)
        # print("hid_bias", self.params.hid_bias.shape)
        # print("output_bias",self.params.output_bias.shape)
        # print("hid_deriv", hid_deriv.shape)
        # print("activations.embedding_layer", activations.embedding_layer.shape)
        # print("word embedding weight", self.params.word_embedding_weights.shape)
    ...
    def my_predict_next_word(self, word1, word2, word3):
        """List the top k predictions for the next word along with their probabilities.
        Inputs:
            word1: The first word as a string.
            word2: The second word as a string.
        ...
        prob = activations.output_layer.ravel()
        idxs = np.argsort(prob)[::-1]   # sort descending
        if '.' in word1 + word2 + word3:
            if self.vocab[idxs[0]] != ".":
                return self.vocab[idxs[0]]
        return self.vocab[idxs[1]]
...
def my_train(embedding_dim, num_hid, config=DEFAULT_TRAINING_CONFIG):
    """This is the main training routine for the language model. It takes two parameters:
...
if __name__ == '__main__':
    ...
    max_ = (1000,None)
    for i in range(200):
        train_model = my_train(16, 128)
        if train_model < max_:
            print('{} better than {}'.format(train_model[0], max_[0]))
            max_ = train_model
        print(i)
    with open('model.pk', "wb") as file:
        cpickle.dump(max_[1], file)
    model = max_[1]
